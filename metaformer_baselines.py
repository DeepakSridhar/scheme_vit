...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, mlp_ratio=4, out_features=None, act_layer=StarReLU, drop=0., bias=False, **kwargs):
        super().__init__()
...
class MetaFormerBlock(nn.Module):
    ...
    def __init__(self, dim,
        ...
        self.mlp = mlp(in_features=dim, mlp_ratio=mlp_ratio, drop=drop)
...
@register_model
def featconvformer_s18(pretrained=False, **kwargs):
    mlp_ratios = 8
    mlp_blocks = GroupLiteAttMlp
    model = MetaFormer(
        depths=[3, 3, 9, 3],
        dims=[64, 128, 320, 512],
        token_mixers=SepConv,
...
@register_model
def featcaformer_s18(pretrained=False, **kwargs):
    mlp_ratios = 8
    mlp_blocks = GroupLiteAttMlp
    model = MetaFormer(
        depths=[3, 3, 9, 3],
...
@register_model
def featpartialcaformer_s12(pretrained=False, **kwargs):
    mlp_ratios = 8
    mlp_blocks = PartialGroupLiteAttMlp
    model = MetaFormer(
        depths=[2, 2, 6, 2],
        dims=[64, 128, 320, 512],
        token_mixers=[SepConv, SepConv, Attention, Attention],
...
@register_model
def featpartialcaformer_s14(pretrained=False, **kwargs):
    mlp_ratios = 8
    mlp_blocks = PartialGroupLiteAttMlp
    model = MetaFormer(
        depths=[2, 2, 8, 2],
...
@register_model
def featpartialcaformer_s18(pretrained=False, **kwargs):
    mlp_ratios = 8
    mlp_blocks = PartialGroupLiteAttMlp
    model = MetaFormer(
        depths=[3, 3, 9, 3],
